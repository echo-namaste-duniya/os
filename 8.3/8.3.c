...
void cscan(int requests[], int n, int head, int prev_head) {
    ...
    // Move to end and wrap around to beginning
...
void clook(int requests[], int n, int head, int prev_head) {
    int total_movement = 0;
    int current = head;
    int sequence[20];  // To store sequence of movements
    int seq_idx = 0;
    ...
    if(start_idx > 0) {
        total_movement += abs(sorted[0] - current);
        current = sorted[0];
        printf(" -> %d", sorted[0]);
        sequence[seq_idx++] = sorted[0];
...
int main() {
    ...
    // Run both algorithms
    cscan(requests, NUM_REQUESTS, head, prev_head);
    printf("\n");
    clook(requests, NUM_REQUESTS, head, prev_head);
